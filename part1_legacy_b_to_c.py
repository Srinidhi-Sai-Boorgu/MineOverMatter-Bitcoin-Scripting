...
def main():
    ...
    with open("tx_info.json", "r") as f:
        tx_info = json.load(f)
    
    address_b = tx_info["address_b"]
    address_c = tx_info["address_c"]
    ...
    unspent_outputs = rpc.listunspent(1, 9999999, [address_b])
    ...
    print(json.dumps(decoded_signed_tx['vin'][0]['scriptSig'], indent=2))
    ...
    with open("tx_info_final.json", "w") as f:
        json.dump({
            "address_a": tx_info["address_a"],
            "address_b": address_b,
            "address_c": address_c,
            "tx_a_to_b": tx_a_to_b,
            "tx_b_to_c": tx_b_to_c,
            "scriptSig": decoded_signed_tx['vin'][0]['scriptSig'],
            "previousScriptPubKey": prev_script_pubkey
        }, f)
    
    print("Final transaction information saved to tx_info_final.json")
